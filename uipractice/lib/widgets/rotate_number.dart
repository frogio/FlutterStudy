...
class _RotateNumberState extends State<RotateNumber>
  ...
  void _getDigitCount() {
    ...
    while (num > 0) {
      num ~/= 10;
      digitCount++;
      if (digitCount % 3 == 0) spaceCount++;
    }

    this.digitCount = digitCount;
    animationOutput = " " * (digitCount + spaceCount);
  }
  ...
  void initState() {
    ...
    animation.addListener(() {
      setState(() {
        ...
        int notation = (curDigit - 1) ~/ 3;
        if (notation < 0) notation = 0;
        if (increment <= value) {
          animationNumber = (curNumber + increment) % 10;
          animationOutput = _replaceCharAt(
              animationOutput,
              numStr.length - curDigit - notation + spaceCount,
              animationNumber.toString());
          increment++;
        }
      });
    });
